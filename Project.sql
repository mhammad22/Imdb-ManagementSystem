...
--select * from Reviews_m
--select * from Reviews_t
--select * from Users
--select * from Watchlist_m
--select * from Watchlist_t
...
AS
BEGIN
SELECT msg,name,mail FROM feedback
END
...
AS
BEGIN
SELECT id,fname,lname,gender,pin,country,dob,email,phoneno FROM Users
END
...
AS
BEGIN
INSERT INTO news VALUES (GETDATE(),@post)
END

GO
CREATE PROCEDURE deletenews
@id1 int, @output int OUTPUT
AS
BEGIN
IF EXISTS (SELECT * FROM news WHERE id=@id1)
BEGIN
DELETE FROM news WHERE id=@id1
SET @output=1
RETURN
END
SET @output=0
...
AS
BEGIN
SELECT id,post_date,data From news
END
--INSERT INTO news VALUES (GETDATE(),'This is me')
--SELECT * FROM News
--DROP PROCEDURE getnews
----------------------------------------------added after
GO
CREATE PROCEDURE updaterating_m
@tid int, @new_rate int
AS
BEGIN
DECLARE @local float
SELECT @local=T.rating FROM Movie T WHERE T.id=@tid
SET @local=(@local+@new_rate)/2
UPDATE Movie SET rating=@local WHERE id=@tid
END

GO
CREATE PROCEDURE addfeeback
@msg1 varchar(1000),@name1 varchar(50),@email varchar(50)
AS
BEGIN
INSERT INTO feedback VALUES (@msg1,@name1,@email)
END
-----------------------------------------------added after
...
as begin
	...
	if exists (SELECT * FROM Admin_DB A WHERE A.id=@id)
	BEGIN
	SET @output=5
	RETURN
	END
...
begin
set @output=1
INSERT INTO Movie VALUES(@id,@name,@duration,@release,@genre,@summary,@rating,@director,'~/Content/na.jpg')
end
else
begin
set @output = 0
...
@id int,@name nvarchar(30), @num_seasons int, @release date, @genre int, @summary nvarchar(300), @rating int, @director int,
@output int output
AS
BEGIN
...
begin
set @output=1
DELETE FROM TV_Shows WHERE TV_Shows.id=@id
RETURN
END
set @output=0
end

--DROP PROCEDURE TV_DEL
...
AS
BEGIN
if not exists(select * from supp_tvshows where id=@id and actor=@actor1)
begin
INSERT INTO supp_tvshows VALUES(@id,@actor1)
set @output=1
RETURN
END
set @output=0
end
--SELECT * FROM supp_tvshows
--EXECUTE supTV_ADD @id=1,@actor=4
--DROP PROCEDURE supTV_ADD
...
AS
BEGIN
if exists (select* from supp_tvshows where id=@id AND actor=@actor1)
begin
set @output=1
DELETE FROM supp_tvshows WHERE supp_tvshows.actor=@actor1 AND supp_tvshows.id=@id
RETURN
END
set @output=0
end
...
AS
BEGIN
if not exists (select * from supp_movies where id=@id AND actor=@actor1)
begin
set @output=1
INSERT INTO supp_movies VALUES(@id,@actor1)
RETURN
END
set @output=0
end
...
AS
BEGIN
if exists(select * from supp_movies where id=@id AND actor=@actor1)
begin
set @output=1
DELETE FROM supp_movies WHERE supp_movies.id=@id AND supp_movies.actor=@actor1
RETURN
END
set @output=0
end
...
begin
DELETE FROM Director WHERE Director.id=@id
set @output=1
RETURN
end
set @output=0
...
begin
DELETE FROM Actor WHERE Actor.id=@id
set @output=1
RETURN
END
set @output=0
